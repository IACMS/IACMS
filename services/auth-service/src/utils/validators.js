...
export function validateCreateUserRequest(body, headerTenantId) {
  ...
  const hasBodyTenant =
    (resolvedTenantCode != null && String(resolvedTenantCode).trim() !== '') ||
    (resolvedTenantId != null && String(resolvedTenantId).trim() !== '');

  if (!hasBodyTenant) {
    if (headerTenantId != null && headerTenantId !== '') {
      const h = String(headerTenantId).trim();
      try {
        resolvedTenantId = validateUUID(h, 'Tenant ID');
      } catch {
  ...
  if (
    (!resolvedTenantCode || String(resolvedTenantCode).trim() === '') &&
    (!resolvedTenantId || String(resolvedTenantId).trim() === '')
  ) {
    throw new ValidationError('Tenant code or organization context is required');
  }
